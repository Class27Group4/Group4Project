...
sudo dnf -y upgrade #If it is not desired to upgrade the server, comment out this line.
sudo dnf -y install wget tree unzip vim java-11-openjdk-devel
...
sudo mv /opt/apache-tomcat-9.0.59 /opt/tomcat9
sudo groupadd tomcat
sudo usermod -a -G tomcat `whoami`
sudo chown -R root:tomcat /opt/tomcat9
