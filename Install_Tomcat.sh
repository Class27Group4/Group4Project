...
sudo chmod g+w /opt/tomcat9/logs/catalina.out # This file needs write permission for whatever reason else you run into an error when trying to use "starttomcat" with using sudo.
...
#	5. Setup access between Maven and Tomcat servers either via ssh or password authentication. Copy the web app from Maven server to Tomcat server using scp (This is a manual operation; Maven should be 
#	   able to do this automatically if setup to do so. We did not go over this in class.)
